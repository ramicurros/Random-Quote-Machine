...
function App() {
    ...
    const backgroundStyle = useRef(null);
    const newQuoteBtn = useRef(null);
    const backgroundColors = ["#EEF1FF", "#D2DAFF", "#CCF3EE", "#F6D186", "#D6E5FA", "#F0D9FF", "#CAF7E3", "#D6D2C4", "#D57E7E", "#FDFAF6", "#E8DFCA", "#A7D2CB", "#B2C8DF", "#CD5D7D", "#D9D7F1"];


    useEffect(() => {
        let num = Math.floor(Math.random() * (QUOTES_ARR.length));
        setAuthor(QUOTES_ARR[num].author);
        setQuote(QUOTES_ARR[num].quote);
        colorChange();
    },[newQuote]);


    function colorChange() {
        setIndex(colorIndex >= backgroundColors.length - 1 ? 0 : colorIndex + 1);
        backgroundStyle.current.style.backgroundColor = backgroundColors[colorIndex];
    }


    return (
        <div id="App" ref={backgroundStyle} className="d-flex flex-column justify-content-center align-items-center app">
            <div id='quote-box' className='d-flex flex-column justify-content-center align-items-center quote-box'>
            ...
            <div id='buttons' className='d-flex flex-row justify-content-center align-items-center buttons'>
                <a id='tweet-quote'
                    ...
                    target="_blank">
                    <img id='twitter-logo' src="https://www.freepnglogos.com/uploads/twitter-logo-png/twitter-logo-white-circle-png-images-23.png" />
                </a>
                <button id='new-quote' ref={newQuoteBtn} className='new-quote' onClick={() => { setNewQuote(!newQuote) }}>New Quote</button>
            </div>
        </div>
    );
}
